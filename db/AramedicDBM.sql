...
CREATE TABLE `historial` (
  `id_paciente` int NOT NULL,
  `question1` char(1) NOT NULL COMMENT 'Pregunta: ¿Padeces alguna enfermedad grave o cronica (diabetes, hipertesion , asma, etc)? (S/N)',
  `question2` char(1) NOT NULL COMMENT 'Pregunta: ¿ Te han realizado alguna cirugía con anterioridad? (S/N)',
  `question3` char(1) NOT NULL COMMENT 'Pregunta: ¿Tomas algun medicamento actualmente? (S/N)',
  `question4` char(1) NOT NULL COMMENT 'Pregunta: ¿Alergias algún medicamento? (S/N)',
  `details1` TEXT NULL COMMENT 'Detalle de enfermedad (si question1 es S)',
  `details2` TEXT NULL COMMENT 'Detalle de cirugía (si question2 es S)',
  `details3` TEXT NULL COMMENT 'Detalle de medicamento (si question3 es S)',
  `details4` TEXT NULL COMMENT 'Detalle de alergia (si question4 es S)',
  
  PRIMARY KEY (`id_paciente`),
  
  CONSTRAINT `fk_historial_paciente` FOREIGN KEY (`id_paciente`) REFERENCES `pacientes` (`id_usuario`),
  
  CONSTRAINT `chk_historial_question1` CHECK ((`question1` IN ('S','N'))),
  CONSTRAINT `chk_historial_question2` CHECK ((`question2` IN ('S','N'))),
  CONSTRAINT `chk_historial_question3` CHECK ((`question3` IN ('S','N'))),
  CONSTRAINT `chk_historial_question4` CHECK ((`question4` IN ('S','N')))
